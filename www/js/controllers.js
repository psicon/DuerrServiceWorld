...
.run(function ($ionicPlatform,$ionicPopup,$rootScope,$ionicHistory,$state,$ionicViewService) {
...
if($state.current.name=="app.browse") {
         ...
         var myPopup = $ionicPopup.show({
            template: "<p style='text-align: center;'>{{ 'EXITAPP' | translate }}?</p>",
            scope: $rootScope,
            buttons: [
                {
    ...
    //cordova.plugins.settings.openSetting("wifi", function(){console.log("opened wifi settings")},function(){console.log("failed to open wifi settings")});
...
.service('SearchValueService', function () {
    ...
    return {
        isLongLang: function (langcode) {
            var longLanguages = ['es', 'it', 'fr', 'dk', 'da', 'ru', 'ch', 'zh'];
...
.service('UserLoginService', function () {
    ...
    return {
        ...
        setShowingIntro: function (val) {
            window.localStorage['showingintro'] = val;
            console.log("showingintro: " + val);
        },
        getShowingIntro: function () {
            if (window.localStorage['showingintro'] == undefined) {
                console.log("undefined intro");
                return false;
            }
            return window.localStorage['showingintro'];
            console.log("showingintro: " + val);
        }
...
.service('RegisterDeviceService', function () {
    ...
    return {
        ...
        setDirectRegister: function (val) {

            window.localStorage['directRegistered'] = val;
            console.log("directRegistered setted: " + val);
        },
        getDirectRegister: function (val) {

            console.log("directRegistered got: " + window.localStorage['directRegistered']);
            return window.localStorage['directRegistered'] + "";
        },
...
// APPINTRO Controller
.controller('IntroCtrl', function (SettingsService, $ionicSideMenuDelegate, $ionicSlideBoxDelegate, HistoryBackService, $ionicViewService, $ionicPlatform, $scope, $ionicModal, $state, $timeout, $ionicLoading, SearchValueService, UserLoginService, $ionicSideMenuDelegate, $ionicPopup, $ionicHistory) {
    ...
    $scope.startApp = function () {
        if (UserLoginService.getShowingIntro() == 1) {
            $ionicHistory.nextViewOptions({
                disableAnimate: true,
                disableBack: true
            });
        ...
        } else if ((UserLoginService.getUserName() != "" && UserLoginService.getUserName() != "guest" && UserLoginService.getRemember()) == true) {
            $ionicHistory.nextViewOptions({
                disableAnimate: true,
                disableBack: true
            });
    ...
    $ionicPlatform.ready(function () {
        ...
        if (remember == true) {
            if (navigator.connection.type == Connection.NONE || navigator.connection.type == Connection.CELL_2G) {
                ...
                UserLoginService.setRemember(1);
                $ionicViewService.nextViewOptions({
                    disableBack: true
                });
                $ionicLoading.hide();
...
.controller('AppCtrl', function (RegisterDeviceService, HistoryBackService, $cordovaFlashlight, $ionicViewService, $ionicPlatform, $scope, $ionicModal, $state, $timeout, $ionicLoading, SearchValueService, UserLoginService, $ionicSideMenuDelegate, $ionicPopup, $ionicHistory) {
    ...
    afterSNScan = function (result2) {
        ...
        if (result2.cancelled) {
            ...
            if (navigator.connection.type == Connection.NONE ||  navigator.connection.type == Connection.UNKNOWN) {
                ...
                SearchValueService.setSearchValue(result2.text);
                $state.go("app.search", {}, {
                    reload: true
                });
            } else {
                RegisterDeviceService.setDirectRegister(0);
    ...
    openSNScanAndroid = function (result) {
        $ionicLoading.hide();
        var myPopup = $ionicPopup.show({
            title: '',
            ...
            buttons: [
                                ...
                                },
                {
                    ...
                    onTap: function (e) {
                        ...
                        cordova.plugins.barcodeScanner.scan(function (result2) {

                            var code = result2.text.charCodeAt(0);
                            if (((code >= 65) && (code <= 90)) || ((code >= 97) && (code <= 122))) {
                                afterSNScan(result2);

                            } else {
                                openSNScanAndroid(result2);
                            }
                        });
                                ...
                                },
                {
                    ...
                    onTap: function (e) {
                        ...
                        cloudSky.zBar.scan({
                            ...
                            function (result2) {
                                var code = result2.charCodeAt(0);
                                if (((code >= 65) && (code <= 90)) || ((code >= 97) && (code <= 122))) {
                                    afterSNScan({
                                        text: result2
                                    });
                                } else {
                                    openSNScanIOS(result2);
                                }
            ...
            cordova.plugins.barcodeScanner.scan(
                function (result) {
                    ...
                    if (result.cancelled) {
                        ...
                        var code = result.text.charCodeAt(0);
                        if (((code >= 65) && (code <= 90)) || ((code >= 97) && (code <= 122))) {
    ...
    $scope.searchproduktedirect = function () {
        ...
        var searchfor = (parent.document.getElementById("suche").value).trim();
        //parent.document.getElementById("suche").value = "";
        var code = searchfor.charCodeAt(0);
        if (((code >= 65) && (code <= 90)) || ((code >= 97) && (code <= 122))) {
            searchfor.nach = searchfor.toUpperCase();
            RegisterDeviceService.setSerial(searchfor);
        } else {
            RegisterDeviceService.setSerial('');
        }
    ...
    $scope.searchprodukte = function (searchfor) {
        ...
        var code = searchfor.nach.charCodeAt(0);
        if (((code >= 65) && (code <= 90)) || ((code >= 97) && (code <= 122))) {
            searchfor.nach = searchfor.nach.toUpperCase();
            RegisterDeviceService.setSerial(searchfor.nach);
        } else {
            RegisterDeviceService.setSerial('');
        }
            ...
            httpReq.post("http://dlc.duerrdental.com/api/", {
                ...
                if (angular.isDefined(statusresponse)) {
                    ...
                    if (angular.isDefined(statusresponse.USER_DATA_VALID)) {
                        ...
                        UserLoginService.setCustomerno(responsedata.customer_no);
                        var remember = ($scope.loginData.remember == true) ? 1 : 0;
                        UserLoginService.setRemember(remember);
...
.controller('getRegdeviceCtrl', function ($ionicPopup, $state, $timeout, $scope, $http, $ionicLoading, $ionicHistory, RegisterDeviceService, SearchValueService, HistoryService, UserLoginService) {
        ...
        var timer = $timeout(
         ...
         var timer = $timeout(
            function () {
                ...
                $ionicLoading.hide();
        var myPopup = $ionicPopup.show({
            template: "<p style='text-align: center;'>{{ 'CHECKWIFI' | translate }}?</p>",
            buttons: [
                {
                    text: '<i class="icon ion-close-circled"></i>'
                },
    ...
    cordova.plugins.settings.openSetting("wifi", function(){console.log("opened nfc settings")},function(){console.log("failed to open nfc settings")});
                                 }
      }
    ]
        });
...
// Get single Article (Scan or direct select)
.controller('getArticleByScanCtrl', function ($ionicViewService, HistoryBackService, $location, $ionicPopup, $state, $timeout, $scope, $http, $ionicLoading, $ionicHistory, RegisterDeviceService, SearchValueService, HistoryService, UserLoginService) {
    //$scope.getArticle =  function() {
    var loaded = HistoryBackService.getScannedLoaded();
    $scope.isGuest = window.localStorage['username'] == 'guest';
    ...
    if (loaded == "true") {
        ...
        if (angular.isUndefined(registereddevices)) {} else {
            for (var d = 0, len = registereddevices.length; d < len; d += 1) {
                if (registereddevices[d][0].serial == RegisterDeviceService.getSerial()) {

                    //$scope.registered = true;
                    $scope.serial_no = registereddevices[d][0].serial;
            ...
            if (!$scope.registered) {
                ...
                $scope.registered = RegisterDeviceService.getDirectRegister() == 1;
            }
            $scope.showService = $scope.isGuest || !$scope.registered;
                ...
                if (data.length < 250) {
                    ...
                    if (langCode != 'en' && langCode != 'de') {
                        ...
                        for (var i = 0; i < documents.length; i++) {
                            ...
                            for (var j = 0; j < filteredDocuments.length; j++) {
                                if (SearchValueService.getDocumentTypeID(documents[i].document_type) == SearchValueService.getDocumentTypeID(filteredDocuments[j].document_type)) {
                                    already = true;
                                    break;
                                }
                            }
                            ...
                            if (!matchFound) {
                                ...
                                filteredDocuments[filteredDocuments.length - 1].document_type = SearchValueService.getDocumentTypeTranslation(applang, documents[i].document_type);
                                if (filteredDocuments[filteredDocuments.length - 1].document_count < documents[i].document_count) {
                                    filteredDocuments[filteredDocuments.length - 1].document_count = documents[i].document_count;
                                }
                            }
                        }
                        ...
                        $scope.articles.data[0].documents = filteredDocuments;
...
.controller('getArticleDocs', function ($ionicPopup, $timeout, $scope, $ionicHistory, $http, $ionicLoading, $state, SearchValueService, UserLoginService, HistoryService, DownloadHistoryService) {
    //$scope.getArticle =  function() {

    /*$scope.$on('$stateChangeStart',
        function (event, toState, toParams, fromState, fromParams) {
            window.localStorage['articledocs'] = "";
            $scope.articledocs = {
                data: []
            };
        });
...
*/
    ...
    var documenttype = window.localStorage['doctype'];
    ...
    var applanguage = window.localStorage['applang'];
    var articledocs = {};
    ...
    httpReq.post("http://dlc.duerrdental.com/api/", {
        ...
        for (var d = 0, len = $scope.articledocs.data.length; d < len; d += 1) {
    ...
    if (applang != 'en') {
        ...
        httpReq.post("http://dlc.duerrdental.com/api/", {
            ...
            for (var d = 0, len = enData.data.length; d < len; d += 1) {
                enData.data[d].file_size = Math.round(enData.data[d].file_size / 1000000 * 100) / 100;
                if (enData.data[d].file_size == 0) {
                    if (enData.data[d].dd_url.indexOf('.pdf') != -1) {
                        enData.data[d].file_preview = "./img/PDF.png"
                    } else {
                        enData.data[d].file_preview = "./img/ico/Doku_Film.png"
            ...
            /*if (window.localStorage['iospatch'] == 0 && ionic.Platform.isIOS()) {
                window.localStorage['iospatch'] == 1;
                $state.go($state.current, {}, {
                    reload: true
                });
            }*/
    ...
    console.log("articledocs", $scope.articledocs);
                ...
                window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fileSystem) {
                    ...
                    var myPopup = $ionicPopup.show({
                        ...
                        buttons: [
                            {
                                text: '<b></b>',
                                type: 'button button-positive  icon ion-close-circled',
                                onTap: function (e) {
...
//alert(1);
    HistoryBackService.setScannedLoaded("false");
    var offlinesearchvalue = SearchValueService.getSearchValue();
    $scope.offlinesearchvalue = offlinesearchvalue;
    $scope.doRefresh = function () {
        if (navigator.connection.type == Connection.NONE ||  navigator.connection.type == Connection.UNKNOWN) {
            $ionicLoading.show();
    ...
    if (navigator.connection.type == Connection.NONE ||  navigator.connection.type == Connection.UNKNOWN) {
        ...
        angular.element(document).ready(function () {
            ...
            console.log("articles before if", $scope.articles);
    ...
    cordova.plugins.settings.openSetting("wifi", function(){console.log("opened nfc settings")},function(){console.log("failed to open nfc settings")});
            ...
            console.log("articles after if", $scope.articles);
    ...
    $scope.filterOfflineSearch = function (xmlItem) {
            if (xmlItem.ArtNr == offlinesearchvalue) {
                //$scope.articlecount++;
                return true;
            }
            if (xmlItem.ArtBezDe.toLowerCase().indexOf(offlinesearchvalue.toLowerCase()) > -1) {
                //$scope.articlecount++;
                return true;
            }
            return false;
...
.controller('HistoryCtrl', function (HistoryBackService, $ionicPopup, $scope, $stateParams, HistoryService, SearchValueService, RegisterDeviceService, $state) {
    ...
    // SearchValueService.setSurgery("");
        //SearchValueService.setSerial("");
    // open opendetails of Device
    $scope.opendetails = function (artno, salesgroup, serial) {
        //alert(artno);
        // Set the selected value in the service for later useage
        SearchValueService.setArticleNo(artno);
        //alert(artno);
        //alert(serial);
        //alert(SearchValueService.getSerial());
        //alert(serial);
        RegisterDeviceService.setSerial(serial);
        SearchValueService.setSurgery("");
        SearchValueService.setSerial(serial);
        SearchValueService.setSalesGroup(salesgroup);
...
.controller('GetregCtrl', function (HistoryBackService, $ionicPopup, $scope, $stateParams, RegisterDeviceService, SearchValueService, $state) {
    ...
    $scope.opendetails = function (articleno, salesgroup, surgery, serialno) {
        ...
        RegisterDeviceService.setSerial(serialno);
        RegisterDeviceService.setDirectRegister(1);
...
.controller('SettingsController', function ($cordovaEmailComposer, $ionicModal, $ionicViewService, HistoryBackService, HistoryService, $ionicPopup, $scope, $stateParams, SettingsService, $state, $window, $ionicLoading, SearchValueService, UserLoginService) {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    onTap: function (e) {
                        ...
                        UserLoginService.setRemember(0);
                        $ionicViewService.nextViewOptions({
                            disableBack: true
                        });
    ...
    $scope.showintroagain = function () {
        ...
        UserLoginService.setShowingIntro(1);
...
.controller('registerDeviceCtrl', function ($cordovaEmailComposer, $cordovaFlashlight, $ionicLoading, $ionicPopup, $scope, $stateParams, $state, RegisterDeviceService, HistoryService, $window, UserLoginService, SearchValueService) {
    var lastarticle = HistoryService.getLastarticle();



    showSNError = function () {
        $ionicLoading.hide();
        var myPopup = $ionicPopup.show({
    ...
    showRefError = function () {
        $ionicLoading.hide();
        var myPopup = $ionicPopup.show({
            title: 'Seriennummer gescannt',
            template: 'Der gescannte Barcode beinhaltet wahrscheinlich nur die Seriennummer. Bitte die Artieklnr scannen.',
            scope: $scope,
            buttons: [
                {
                    text: '<i class="icon ion-checkmark"></i>',
    ...
    var registerserial = RegisterDeviceService.getSerial();

    if (registerserial == undefined || registerserial == 'undefined') {
        $scope.registerserial = '';
    } else {
        $scope.registerserial = registerserial;
    }
    ...
    $scope.registerformsubmit = function () {
        ...
        if (devicePlatform == "Android") {
            ...
            var alertPopup = $ionicPopup.alert({
             ...
             RegisterDeviceService.setSerial($scope.serialno);
            $state.go("app.scanned",{},{reload:true});
            ...
            RegisterDeviceService.setArtno($scope.registerartnr);
            RegisterDeviceService.setSerial($scope.serialno);
            console.log("Ref_NR:", $scope.registerartnr);
            console.log("SN", $scope.serialno);
            $state.go("app.scanned",{},{reload:true});
        ...
        var email = {
            ...
            body: "Praxis: " + $scope.surgery + "<br>" +
                ...
                "Kundennummer: " + UserLoginService.getCustomerno() + "<br>" +
...
.controller('ServiceCtrl', function ($cordovaEmailComposer, $window, $ionicPopup, $scope, $stateParams, RegisterDeviceService, HistoryService, $cordovaCamera, SearchValueService, UserLoginService) {
    ...
    $scope.serviceformsubmit = function () {
        ...
        for (var i = attachments.length; i >= 0; i--) {
            if (attachments[i] == undefined) {
                attachments.splice(i, 1);
            }
        }

        //alert("after attachments - " + attachments.length);
        ...
        var email = {
            ...
            body: "UserID: " + UserLoginService.getUserID() + "<br>" +
                ...
                "Geräteartnr: " + RegisterDeviceService.getArtno() + "<br>" +
                "GeräteSeriennr: " + RegisterDeviceService.getSerial() + "<br>",
            isHtml: true
        };
...
.controller('DragController', function ($scope, $stateParams, $ionicSideMenuDelegate, UserLoginService) {
     $ionicSideMenuDelegate.canDragContent(false);
    $scope.username = UserLoginService.getUserName();
    $scope.password = UserLoginService.getUserPassword();
    UserLoginService.setShowingIntro(0);

    if (UserLoginService.getUserName() == 'guest' || UserLoginService.getRemember() != 1) {
        $scope.loginData.username = '';
        $scope.loginData.password = '';
    } else {
        $scope.loginData.username = UserLoginService.getUserName();
        $scope.loginData.password = UserLoginService.getUserPassword();
    }
 })

//browsecontroller
.controller('BrowseController', function ($scope, $stateParams, UserLoginService, $ionicLoading, $ionicPopover) {
